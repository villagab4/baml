...
class AbstractLLMProvider(BaseProvider, abc.ABC):
  ...
  @typing.final
  def _start_run(self, prompt: typing.Union[str, typing.List[LLMChatMessage]]) -> None:
  ...
  def _end_run(self, response: LLMResponse) -> None:
    create_event(
      "llm_request_end",
      {
        ...
        "meta": json.dumps(
          response.meta,
          default=lambda x: (x.model_dump() if isinstance(x, BaseModel) else str(x)),
        ),
      },
    )
  ...
  def _set_args(self, **kwargs: typing.Any) -> None:
    ...
    self.__client_args = {
      k: v if k not in self.__redactions else _redact(v) for k, v in kwargs.items()
    }
