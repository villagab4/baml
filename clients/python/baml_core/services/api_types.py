...
class LogSchema(BaseModel):
  ...
  event_type: Literal["log", "func_llm", "func_prob", "func_code"] = Field(frozen=True)
  ...
  def to_pretty_string(self) -> str:
    ...
    if metadata := self.metadata:
      ...
      for k, v in metadata.input.prompt.template_args.items():
        prompt = prompt.replace(k, colorama.Style.BRIGHT + v + colorama.Style.NORMAL)
      pp.extend(
        [
      ...
      if llm_output := metadata.output:
        ...
        pp.append(colorama.Style.DIM + llm_output.raw_text + colorama.Style.NORMAL)
        pp.append(separator)
    ...
    if error := self.error:
      pp.append("Error")
      pp.append(colorama.Style.BRIGHT + str(error.message) + colorama.Style.NORMAL)
      pp.append(separator)
    if len(pp) == 0:
      return ""
    cached_string = ""
