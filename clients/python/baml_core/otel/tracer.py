...
def _trace_internal(func: F, **kwargs: typing.Any) -> F:
  ...
  if func.__annotations__.get("baml_is_stream") is True:
    # Assert the return type of the function is an AsyncStream
    assert (
      func.__annotations__.get("return") == AsyncStream
    ), f"Expected return type to be AsyncStream, got {func.__annotations__.get('return')}"
