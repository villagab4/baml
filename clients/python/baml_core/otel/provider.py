...
class CustomBackendExporter(SpanExporter):
  ...
  def export(self, spans: typing.Sequence[ReadableSpan]) -> SpanExportResult:
    ...
    items = list(
      chain.from_iterable(
        event_to_log(span, project_id=self.__project_id, process_id=self.__process_id)
        for span in spans
      )
    )
...
class BamlSpanContextManager:
  ...
  def __enter__(self) -> "BamlSpanContextManager":
    ...
    current_attributes = attributes_context.get()
    span_attributes = current_attributes.get(self.parent_id, {"__BAML_ID__": span_id})
    current_attributes[span_id] = span_attributes
    attributes_context.set(current_attributes)
    return self
  ...
  def __exit__(
    ...
    if span_history is not None:
      self.span.set_attribute("root_span_ids", list(map(lambda m: m[0], span_history)))
